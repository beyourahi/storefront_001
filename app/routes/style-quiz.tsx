...
export const meta: Route.MetaFunction = () => {
    return [{title: "Style Quiz | Discover Your Style"}];
};
...
/* ── Step definitions ── */
const STEPS: {key: StepKey; question: string; options: PillOption[]}[] = [
    {
        key: "fit",
...
export default function StyleQuiz() {
    ...
    // Load saved profile from localStorage after mount (SSR-safe)
    useEffect(() => {
        setMounted(true);
        try {
    ...
    // Persist profile key when result is computed
    useEffect(() => {
        if (!result) return;
        try {
    ...
    if (result) {
        return (
            <div className="min-h-screen bg-[var(--surface-canvas)] text-[var(--text-primary)]">
                <PageBreadcrumbs customTitle="Style Quiz" />
    ...
    return (
        <div className="min-h-screen bg-[var(--surface-canvas)] text-[var(--text-primary)]">
            ...
            <div className="mx-auto max-w-2xl px-4 py-10 sm:py-16">
                {/* Welcome back banner — only shown before quiz starts and after mount */}
                {mounted && savedProfile && currentStep === 0 && (
                    ...
                    >
                        ...
                        <button
                            onClick={() => {
                                void navigate(`/search?q=${encodeURIComponent(savedProfile)}`);
                            }}
                ...
                >
                    ...
                    {/* Pill options — full-width stacked */}
                        ...
                        {step?.options.map(option => {
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        "relative flex items-center gap-4 py-4 px-5 rounded-xl text-left transition-all duration-200 active:scale-[0.99] border-l-4",
                                        "w-full"
                                    )}
                                    style={{
                ...
                <div className="flex items-center justify-between mt-10 gap-3">
                    ...
                    >
                        {currentStep === totalSteps - 1 ? "See my style" : "Next"}
