...
export const loader = async ({params, context}: Route.LoaderArgs) => {
    ...
    if (!isAuthenticated) {
        return remixData(
            {subscription: null, isAuthenticated: false as const},
            {
                headers: {
    ...
    return remixData(
        {subscription, isAuthenticated: true as const},
        {
            headers: {
                "Set-Cookie": await context.session.commit(),
                "Cache-Control": "no-cache, no-store, must-revalidate"
...
export default function SubscriptionDetail() {
    const {subscription, isAuthenticated} = useLoaderData<typeof loader>();
    ...
    if (!isAuthenticated || !subscription) {
        return (
            <AuthRequiredFallback
                message="Sign in to view subscription details"
                description="Access your subscription details, manage billing cycles, and update your preferences."
            />
        );
    }

    const statusConfig =
        SUBSCRIPTION_STATUSES[subscription.status as keyof typeof SUBSCRIPTION_STATUSES] ??
                    ...
                    {subscription.lines.nodes.map((line: SubscriptionLine) => (
                        <div key={line.id} className="flex gap-4 py-4 first:pt-0 last:pb-0">
                            ...
                            <div className="flex flex-1 flex-col justify-center gap-1">
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Qty: {line.quantity} • <Money data={line.currentPrice as any} /> each
                                </p>
                            </div>
                            <div className="flex items-center">
                                <span className="font-medium">
                                    <Money data={line.lineDiscountedPrice as any} />
                                </span>
                            </div>
                        </div>
                    ))}
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2">
                        ...
                        {subscription.deliveryPrice && parseFloat(subscription.deliveryPrice.amount) > 0 && (
                            <p className="text-sm text-muted-foreground">
                                + <Money data={subscription.deliveryPrice as any} /> delivery
                            </p>
                        )}
                    </CardContent>
                </Card>
                        ...
                        {subscription.orders.nodes.map((order: SubscriptionOrder) => (
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <span className="font-medium">
                                        <Money data={order.totalPrice as any} />
                                    </span>
                                </div>
                            </div>
                        ))}
