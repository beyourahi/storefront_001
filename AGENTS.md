...
│   │   ├── product/               # PDP components (gallery, options, purchase, AgentProductBrief, CatalogExtensionDisplay, SimilarItems)
│   │   ├── pwa/                   # Install prompts, SW update banner
│   │   ├── search/                # Search results + controls; includes SearchEmptyState for zero-result states
│   │   ├── sections/              # Reusable page sections
│   │   └── ProductLightbox/       # Fullscreen image lightbox
│   ├── lib/                       # Shared utilities
│   │   ├── agentic/               # UCP/MCP agentic commerce layer (agent-server, mcp-router, catalog-shapes, quizzes, etc.)
│   │   ├── color/                 # OKLCH + contrast helpers
│   │   ├── performance/           # Image optimization helpers
│   │   ├── product/               # Product parsing, pricing, variants
│   │   ├── queries/               # Agent-optimized GraphQL query fragments
...
**Sitemap**: `app/routes/sitemap[.]custom[.]xml.tsx` (custom sitemap for routes outside Shopify's generated sitemap index — `/faq`, `/gallery`, `/sale`, `/changelog`, `/wishlist`, `/compare`, `/gift-finder`, `/stories`, `/style-quiz`, `/policies`)
**Agentic / MCP Layer**: `app/lib/agentic/types.ts` (UCP/MCP type definitions — `UcpProfile`, `UcpCapability`, `McpToolRegistry`, `AgentContext`, `AgentProfile`), `app/lib/agentic/agent-request.ts` (`isAgentRequest()` — detects AI agent requests via `?agent=true` param or `Accept: application/x-ucp+json` header), `app/lib/agentic/agent-auth.ts` (`verifyAgentBearer()` — validates agent bearer tokens for authenticated MCP endpoints), `app/lib/agentic/agent-context.ts` (`buildAgentContext()` — assembles agent context from Hydrogen context for MCP tool handlers), `app/lib/agentic/agent-server.ts` (server-level UCP handler — intercepts product page requests from agents before React Router rendering; called from `server.ts`), `app/lib/agentic/ucp-profile.ts` (`buildUcpProfile()` — constructs the UCP merchant profile for `/.well-known/ucp`), `app/lib/agentic/catalog-shapes.ts` (`toUcpProductPage()` — transforms Shopify product GraphQL responses into UCP-structured JSON), `app/lib/agentic/compare.ts` (`buildComparisonMatrix()` — normalized comparison matrix for `compare_products` MCP tool and `/compare` route), `app/lib/agentic/affinity.ts` (server-side affinity scoring — re-ranks collection products by customer order history; degrades gracefully for anonymous users), `app/lib/agentic/attribute-normalizer.ts` (`normalizeAttribute()` — maps Shopify variant options and metafields to Schema.org-aligned vocabulary), `app/lib/agentic/structured-data.ts` (agent-readable meta-tag emission for SCE fields), `app/lib/agentic/agent-id-hash.ts` (`agentIdHash()` — privacy-safe SHA-256 truncated hash of agent identity for observability logging; first 16 hex chars, not reversible), `app/lib/agentic/agent-surface.ts` (`deriveAgentSurface()`, `AgentSurface` — classifies request origin as MCP session, AI referer, permalink, or human; consumed by root.tsx and banner components), `app/lib/agentic/jwks-cache.ts` (`getJwks()` — in-memory 1-hour TTL JWKS cache used by agent-auth.ts for bearer token verification), `app/lib/agentic/observability.ts` (`emitAgentEvent()`, `routeFromRequest()` — dual-target structured event emitter for agentic observability: console.log JSON + Analytics Engine when `AGENT_ANALYTICS` binding present; privacy-safe field allowlist, never logs PII), `app/lib/agentic/ucp-catalog-types.ts` (`UcpProduct`, `UcpVariant`, `UcpMoney`, `UcpMedia`, `UcpProductPage`, `UcpLookupBatch` — UCP catalog type definitions extracted from catalog-shapes), `app/lib/agent-surface-context.tsx` (`AgentSurfaceProvider`, `useAgentSurface()` — React context for propagating `AgentSurface` state to banner and agent-adaptive components), `app/lib/agentic/mcp-router.ts` (`routeMcpRequest()` — JSON-RPC 2.0 dispatcher for registered MCP tools), `app/lib/agentic/mcp-tools/` (MCP tool implementations — `storefront/`: search-catalog, get-product, compare-products, lookup-catalog, recommend-similar, recommend-complementary, get-story-feed, list-sort-options, search-suggest; `policies/`: policy-corpus, search-policies-and-faqs), `app/lib/agentic/quizzes/` (`gift-finder.ts` — gift recommendation logic, `style-fit.ts` — style profile + `resolveStyleQuery()`)
**Agent GraphQL Queries**: `app/lib/queries/product.ts` (`AGENT_PRODUCT_FRAGMENT` — agent-optimized product fields), `app/lib/queries/search.ts` (`AGENT_SEARCH_PRODUCT_FRAGMENT`), `app/lib/queries/predictive-search.ts` (`AGENT_PREDICTIVE_PRODUCT_FRAGMENT`), `app/lib/queries/lookup.ts` (`AGENT_LOOKUP_PRODUCT_FRAGMENT` — nodes lookup for recently viewed agent tools), `app/lib/queries/policy-corpus.ts` (`POLICIES_CORPUS_QUERY` — fetches all shop policies for the MCP policy tool)
**Agentic Routes**: `app/routes/[llms.txt].tsx` (AI transparency manifest — `llms.txt` convention, documents storefront capabilities for AI crawlers), `app/routes/[.]well-known.ucp.tsx` (UCP merchant profile — discovery endpoint used by agents during capability negotiation), `app/routes/api.mcp.tsx` (public MCP endpoint — JSON-RPC 2.0 over HTTP for policy/FAQ tools; no auth required), `app/routes/api.ucp.mcp.tsx` (authenticated MCP endpoint — catalog, cart, comparison, affinity tools; requires agent bearer token)
**Discovery Routes**: `app/routes/compare.tsx` (`/compare` — side-by-side product comparison page; accepts `?ids=` params), `app/routes/gift-finder.tsx` (`/gift-finder` — guided gift recommendation quiz), `app/routes/stories.tsx` (`/stories` — full-screen cinematic story-style shopping experience), `app/routes/style-quiz.tsx` (`/style-quiz` — style profile quiz with personalized product recommendations), `app/routes/policies._index.tsx` (`/policies` — index listing all shop policies)
...
**Infrastructure**: `app/lib/rate-limit.ts` (`createRateLimiter()`, `getClientIP()`, `getRateLimitResponse()` — in-memory sliding-window rate limiter for API routes; per-isolate only, not globally shared across Workers instances), `app/lib/redirect.ts` (`redirectIfHandleIsLocalized()` — issues a redirect when the URL handle differs from the localized resource handle returned by Shopify), `app/lib/policy.ts` (`parsePolicySections()` — parses Shopify shop policy HTML into structured sections for the policy page layout component), `app/lib/cart-permalink.ts` (pure helpers for constructing and parsing Shopify cart permalink URLs — used by agent MCP responses and share/reorder flows), `app/lib/recently-viewed-tools.ts` (agent tool bridge — exposes recently-viewed product IDs from the request cookie for server-side `lookup_catalog` agent tool calls), `app/lib/seo-breadcrumbs.ts` (`deriveBreadcrumbsFromPath()` — pure breadcrumb derivation from URL path; shared by `PageBreadcrumbs.tsx` visual component and JSON-LD emitters)
...
This storefront implements a **Universal Commerce Protocol (UCP) + Model Context Protocol (MCP)** layer that lets AI agents browse the catalog, compare products, manage carts, and read policies — all over JSON-RPC 2.0.
...
| `app/lib/agentic/compare.ts` | `buildComparisonMatrix()` — normalized product comparison for `/compare` route and MCP tool |
...
| `app/lib/agentic/mcp-tools/` | Tool implementations: `storefront/` (search-catalog, get-product, compare-products, lookup-catalog, recommend-similar, recommend-complementary, get-story-feed, list-sort-options, search-suggest), `policies/` (policy-corpus, search-policies-and-faqs) |
| `app/lib/agentic/quizzes/` | `gift-finder.ts`, `style-fit.ts` + `resolveStyleQuery()` — quiz logic for `/gift-finder` and `/style-quiz` |
...
| `app/routes/compare.tsx` | `/compare?ids=` | Side-by-side product comparison page |
| `app/routes/gift-finder.tsx` | `/gift-finder` | Guided gift recommendation quiz |
| `app/routes/stories.tsx` | `/stories` | Full-screen cinematic story-style shopping experience |
| `app/routes/style-quiz.tsx` | `/style-quiz` | Style profile quiz with personalized product recommendations |
| `app/routes/policies._index.tsx` | `/policies` | Index listing all shop policies |

### Critical Pattern: Agent Detection

```typescript
// server.ts — agents are intercepted before React Router rendering
import {handleAgentRequest} from "~/lib/agentic/agent-server";

// Returns a UCP JSON Response for agent requests, or null for human browsers
const agentResponse = await handleAgentRequest(request, context);
