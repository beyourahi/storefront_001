...
// NOTE: CuratedCollectionNode, CuratedTab, CuratedCollectionsData, CuratedCollectionsProps
// are omitted — storefront_001 does not define a CuratedCollectionsQuery in its GraphQL schema.
// If curated collections are added later, uncomment and import CuratedCollectionsQuery.
...
export interface SiteSettings {
    ...
    // list.url fields — parsed to string[]; [] when absent
    // ─────────────────────────────────────────────────────────────────────────
    /** Google Maps embed src URLs (one per location) */
 ...
 * Public API surface — exposes only what consumers need.
 * Internal derivation (seeds, diagnostics, scheme, colors, radius) lives in ResolvedTheme (theme-utils.ts).
 */
