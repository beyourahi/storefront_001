...
export const WishlistCount = ({className, iconSize = 24}: WishlistCountProps) => {
    ...
    return (
        ...
        >
            ...
            {count > 0 && (
                <span
                    className="absolute right-0 top-0 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-red-500 px-1 text-xs font-semibold text-white transition-all duration-200 animate-in fade-in zoom-in"
                    aria-hidden="true"
                >
                    {count > 99 ? "99+" : count}
                </span>
